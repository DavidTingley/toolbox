...
if ~isempty(exclude)
    ...
    exclude = floor(exclude * 1000/Fs); % in ms
...
edges = risefall_temp(:); 
repeated_edges = find([edges(1); diff(edges)] < 0); % 0 can be replaced with a minimum inter-event interval, if we want to merge the events that are close together

secondaryPeriods(floor(repeated_edges/2)+1 , :) = [];


% converting from milisecond to sampling points
