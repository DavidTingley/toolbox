...
for smoothing = 1:round(nBins/2)
    ...
    for cond = 1:length(unique(behavior.events.trialConditions))
        if sum(behavior.events.trialConditions==cond)>=7
%         figure(cond)
        % smooth data..
        for cell = 1:length(spikes.times)
           for trial = 1:size(binnedfiringMaps.phaseMaps{cond},2)
               binnedfiringMaps.phaseMaps_smooth{cond}(cell,trial,:) = circ_smoothTS(squeeze(binnedfiringMaps.phaseMaps{cond}(cell,trial,:)),smoothing,'method','mean');
