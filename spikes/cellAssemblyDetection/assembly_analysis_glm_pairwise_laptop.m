...
% list = list(3:end);
load('/home/david/nitzlabdata/w.mat','w');
load('/home/david/nitzlabdata/spike_overlap_percentages_within_3ms.mat','list_percentages');

%% should run once to do all glm analyses
rats = ['AA8';'DN3';'NS1'; 'NS2';'NS3';'NS4';'NS5';'NS8'];
load('/home/david/Dropbox/bf_dataset_old_all_distros.mat','file','num_cells');
ncells = num_cells;
...
for recording = 99:-1:1
       ...
       if e ~= 0
           mkdir(['/home/david/nitzlabdata/assemblies_check/'   rats(rat,:) '/'  sprintf('%02.0f',recording)])
       ...
       parfor win = 0:600
           if ~exist(['/media/david/Iomega HDD/David/assemblies_check/'   rats(rat,:) '/'  sprintf('%02.0f',recording)  '/'...
...
%     
 ...
 [xc{cell_pair_num}] = glm_for_trials_xcorr_pairwise(s_all,s_all_norm,cell_num,cnum2);
...
if ~isempty(cell_list)
        ...
        save(['/home/david/nitzlabdata/assemblies_check/' rats(rat,:) '/' sprintf('%02.0f',recording)  '/'...
           rats(rat,:) '_rec_' sprintf('%02.0f',recording) '_winsize_'  num2str(win) '.mat'],...
           'wire','*list','xc')%,'*control*','dev*','cnum2')%,'*ll*','params','*yhat*') % *stats*
end
    end
