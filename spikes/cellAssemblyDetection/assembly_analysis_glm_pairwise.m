...
% list = dir('//media/david/Iomega HDD/David/recording_trains_pairwise/');
% list = list(3:end);
load('/media/david/Iomega HDD/David/w.mat','w');
load('/media/david/Iomega HDD/David/spike_overlap_percentages_within_3ms.mat','list_percentages');

%% should run once to do all glm analyses
rats = ['AA8';'DN3';'NS1'; 'NS2';'NS3';'NS4';'NS5';'NS8'];
load('/media/david/Iomega HDD/David/ratemaps_10012012_bf.mat','file','num_cells');
ncells = num_cells;

for rat = 1:8
for recording = 1:99

       e = exist(['/media/david/Iomega HDD/David/recording_trains_pairwise/' rats(rat,:) '/' sprintf('%02.0f',recording)  '/'...
       ...
       if e ~= 0
           mkdir(['/media/david/Iomega HDD/David/assemblies_trial_shuffled/' rats(rat,:) '/' sprintf('%02.0f',recording)])
       ...
       parfor win = 0:600
           if ~exist(['/media/david/Iomega HDD/David/assemblies_trial_shuffled/' rats(rat,:) '/' sprintf('%02.0f',recording)  '/'...
           rats(rat,:) '_rec_' sprintf('%02.0f',recording) '_winsize_'  num2str(win) '.mat'])
%         t = dir(['/media/david/Iomega HDD/David/assemblies/' rats(rat,:) '/' sprintf('%02.0f',recording)  '/'...
%            rats(rat,:) '_rec_' sprintf('%02.0f',recording) '_winsize_'  num2str(win) '.mat']);
%        
...
%     % HMM, linear regression, GLM  
    ...
    load(['/media/david/Iomega HDD/David/recording_trains_pairwise/' rats(rat,:) '/' sprintf('%02.0f',recording)  '/'...
           rats(rat,:) '_rec_' sprintf('%02.0f',recording) '_winsize_'  num2str(win) '.mat'],...
...
%     [yhat{cell_pair_num}, yhat_justpos{cell_pair_num}, yhat_justpeers{cell_pair_num}, yhat_control{cell_pair_num}...
    ...
    ,dev{cell_pair_num},dev_justpos{cell_pair_num},dev_justpeers{cell_pair_num},dev_control{cell_pair_num},...
     ] = glm_for_trials_pairwise_trial_shuffled(s_all,s_all_norm,cell_num,cnum2);
...
if ~isempty(cell_list)
        ...
        save(['/media/david/Iomega HDD/David/assemblies_trial_shuffled/' rats(rat,:) '/' sprintf('%02.0f',recording)  '/'...
           rats(rat,:) '_rec_' sprintf('%02.0f',recording) '_winsize_'  num2str(win) '.mat'],...
           '*control*','dev*','cnum2','wire','*list')%,'params','*yhat*') % *stats*
end
    end
