...
sigs = '/media/nitzlab/data/Dropbox/Dropbox/Spike_trains_parietal/';
% load('C:\Users\Nitz Lab 2\Dropbox_tingley\Dropbox\Dropbox\num_cells.mat')
load('/media/nitzlab/data/Dropbox/Dropbox/num_cells_parietal')
ncells = num_cells;
rec_start = 6;
...
for rec = rec_start % 1:length(num_cells) 
...
%     if ~exist(['/media/nitzlab/Iomega HDD/David/recording_trains_pairwise_parietal/' ratname ...
%         '/' num2str(recording)])  % checks if data has already been generated for recording
    mkdir(['/media/nitzlab/Iomega HDD/David/recording_trains_pairwise_parietal/' ratname ...
        '/' num2str(recording)])
    
    parfor i = 0:600 % ms window
         [s_all s_all_norm] = paradd_pairwise(ncells,rec,i,s);
          fsave(['/media/nitzlab/Iomega HDD/David/recording_trains_pairwise_parietal/' ...
