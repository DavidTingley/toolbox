...
for trial_num = 1:size(s_all,2)
                ...
                xpos = [1:size(s_all,3)];
                y = squeeze(s_all_norm(cell_num,r(trial_num),:))';
...
%                 yhat(trial_num,:) = glmval(results,xboth','identity');
                ...
                %% just peers
                xpeers = [squeeze(s_all(cnum2,trial_num,:))'];
...
%                 yhat_justpeers(trial_num,:) = glmval(results_justpeers,xpeers','identity');
                ...
                xcontrol = [squeeze(s_all(cnum2,r(trial_num),:))'];
...
%                 yhat_control(trial_num,:) = glmval(results_control,xcontrol','identity');
                ...
                params = [mean(s_all_norm(cell_num,r(trial_num),:)),std(s_all_norm(cell_num,r(trial_num),:))];
...
%                 controll2(trial_num) = full(normlike(params,(yhat_control(trial_num,:))));
                ll2(trial_num) = full(normlike(params,s_all_norm(cell_num,r(trial_num),:)));
