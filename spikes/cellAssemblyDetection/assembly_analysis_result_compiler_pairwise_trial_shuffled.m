...
load('/home/david/Dropbox/Nitz lab Shared/BF_categories_paper/bf_dataset.mat','file','num_cells');
file = file;
num_cells = num_cells;
...
for rat = 1:8
   for rec = 1:99 % can be a parfor loop if glm_test isnt running 
       ...
       if exist(['/media/david/Iomega HDD/David/assemblies_trial_shuffled/' rats(rat,:) '/' sprintf('%02.0f',rec) ])
        e = dir(['/media/david/Iomega HDD/David/assemblies_trial_shuffled/' rats(rat,:) '/' sprintf('%02.0f',rec) '/'...
...
save assembly_population_results_trial_shuffled
end
...
function    [dev_new dev_justpeers_new dev_justpos_new dev_control_new ind val indc valc cell_list]...
    ...
    for win = 1:601
      ...
      load(['/media/david/Iomega HDD/David/assemblies_trial_shuffled/' rats(rat,:) '/' sprintf('%02.0f',rec) '/'...
