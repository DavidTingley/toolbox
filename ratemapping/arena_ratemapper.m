...
failure_starts=find(spiral_events(:,3)==2);
...
no_successes=length(success_starts);
no_failures=length(failure_starts);

g=length(success_starts);
gg=length(success_nose);
ggg=length(success_ends);
h=length(failure_starts);
hh=length(failure_nose);
hhh=length(failure_ends);
if g==gg & gg==ggg
...
for i=1:g
    success_markers_points(i,:)=[spiral_events(success_starts(i),1)-120 spiral_events(success_starts(i),1) spiral_events(success_nose(i),1) spiral_events(success_ends(i),1) spiral_events(success_ends(i),1)+120];
end
for i=1:h
    failure_markers_points(i,:)=[spiral_events(failure_starts(i),1)-120 spiral_events(failure_starts(i),1) spiral_events(failure_nose(i),1) spiral_events(failure_ends(i),1) spiral_events(failure_ends(i),1)+120];
end
for i=1:g
    success_markers_times(i,:)=[spiral_events(success_starts(i),2)-2 spiral_events(success_starts(i),2) spiral_events(success_nose(i),2) spiral_events(success_ends(i),2) spiral_events(success_ends(i),2)+2];
end
for i=1:h
    failure_markers_times(i,:)=[spiral_events(failure_starts(i),2)-2 spiral_events(failure_starts(i),2) spiral_events(failure_nose(i),2) spiral_events(failure_ends(i),2) spiral_events(failure_ends(i),2)+2];
end
...
% 
% for i=1:length(success_starts)
% for j=spiral_events(success_starts(i),1)-120:spiral_events(success_ends(i),1)+120
% pos_occ_success(pos(j,2),pos(j,3))=pos_occ_success(pos(j,2),pos(j,3))+1;
% end
% end
% 
% for i=1:length(failure_starts)
% for j=spiral_events(failure_starts(i),1)-120:spiral_events(failure_ends(i),1)+120
...
% success_starts_peth=zeros(no_cells,length(success_starts),41);
% failure_starts_peth=zeros(no_cells,length(failure_starts),41);
...
% success_starts_peth_smooth=zeros(no_cells,length(success_starts),41);
% failure_starts_peth_smooth=zeros(no_cells,length(failure_starts),41);
...
%     success_starts_peth(loop,i,j+21)=length(x)*20;
...
%     failure_starts_peth(loop,i,j+21)=length(x)*20;
...
% success_starts_peth_smooth(loop,j,:)=filtfilt(y3_peth,1,success_starts_peth(loop,j,:));
...
% for loop=1:no_cells
%     for j=1:no_failures
% failure_starts_peth_smooth(loop,j,:)=filtfilt(y3_peth,1,failure_starts_peth(loop,j,:));
...
% 
% success_starts_peth_means_stds_stes=zeros(no_cells,41,3);
% failure_starts_peth_means_stds_stes=zeros(no_cells,41,3);
...
% 
% success_starts_peth_smooth_means_stds_stes=zeros(no_cells,41,3);
% failure_starts_peth_smooth_means_stds_stes=zeros(no_cells,41,3);
...
% for loop=1:no_cells
%     for j=1:41
%     success_starts_peth_means_stds_stes(loop,j,1)=mean(success_starts_peth(loop,:,j));
%     success_starts_peth_means_stds_stes(loop,j,2)=std(success_starts_peth(loop,:,j));
%     success_starts_peth_means_stds_stes(loop,j,3)=std(success_starts_peth(loop,:,j))/(sqrt(length(success_starts)-1));
...
%     failure_starts_peth_means_stds_stes(loop,j,1)=mean(failure_starts_peth(loop,:,j));
%     failure_starts_peth_means_stds_stes(loop,j,2)=std(failure_starts_peth(loop,:,j));
%     failure_starts_peth_means_stds_stes(loop,j,3)=std(failure_starts_peth(loop,:,j))/(sqrt(length(failure_starts)-1));
...
%     
%     success_starts_peth_smooth_means_stds_stes(loop,j,1)=mean(success_starts_peth_smooth(loop,:,j));
%     success_starts_peth_smooth_means_stds_stes(loop,j,2)=std(success_starts_peth_smooth(loop,:,j));
%     success_starts_peth_smooth_means_stds_stes(loop,j,3)=std(success_starts_peth_smooth(loop,:,j))/(sqrt(length(success_starts_peth_smooth)-1));
...
%     failure_starts_peth_smooth_means_stds_stes(loop,j,1)=mean(failure_starts_peth_smooth(loop,:,j));
%     failure_starts_peth_smooth_means_stds_stes(loop,j,2)=std(failure_starts_peth_smooth(loop,:,j));
%     failure_starts_peth_smooth_means_stds_stes(loop,j,3)=std(failure_starts_peth_smooth(loop,:,j))/(sqrt(length(failure_starts_peth_smooth)-1));
...
% success_starts_peth_normed_mag=zeros(no_cells,length(success_starts),41);
% success_nose_peth_normed_mag=zeros(no_cells,length(success_starts),41);
% success_ends_peth_normed_mag=zeros(no_cells,length(success_starts),41);
% success_starts_peth_smooth_normed_mag=zeros(no_cells,length(success_starts),41);
% success_nose_peth_smooth_normed_mag=zeros(no_cells,length(success_starts),41);
% success_ends_peth_smooth_normed_mag=zeros(no_cells,length(success_starts),41);
% failure_starts_peth_normed_mag=zeros(no_cells,length(failure_starts),41);
% failure_nose_peth_normed_mag=zeros(no_cells,length(failure_starts),41);
% failure_ends_peth_normed_mag=zeros(no_cells,length(failure_starts),41);
% failure_starts_peth_smooth_normed_mag=zeros(no_cells,length(failure_starts),41);
% failure_nose_peth_smooth_normed_mag=zeros(no_cells,length(failure_starts),41);
% failure_ends_peth_smooth_normed_mag=zeros(no_cells,length(failure_starts),41);
% for loop=1:no_cells
%     success_starts_peth_normed_mag(loop,:,:)=(success_starts_peth(loop,:,:)-mean_rates_overall(loop))./(success_starts_peth(loop,:,:)+mean_rates_overall(loop));
...
%     failure_starts_peth_normed_mag(loop,:,:)=(failure_starts_peth(loop,:,:)-mean_rates_overall(loop))./(failure_starts_peth(loop,:,:)+mean_rates_overall(loop));
...
%     success_starts_peth_smooth_normed_mag(loop,:,:)=(success_starts_peth_smooth(loop,:,:)-mean_rates_overall(loop))./(success_starts_peth_smooth(loop,:,:)+mean_rates_overall(loop));
...
%     failure_starts_peth_smooth_normed_mag(loop,:,:)=(failure_starts_peth_smooth(loop,:,:)-mean_rates_overall(loop))./(failure_starts_peth_smooth(loop,:,:)+mean_rates_overall(loop));
...
success_timenorm_rates=zeros(no_cells,length(success_starts),69);
failure_timenorm_rates=zeros(no_cells,length(failure_starts),69);
...
for loop=1:no_cells
    ...
    for i=1:length(success_starts)
        ...
        for j=-23:-1
            ...
            success_timenorm_rates(loop,i,j+24)=length(x)*11.4943;
        end
        % last 2 seconds
        for j=1:23
            x=find(tfile>(success_markers_times(i,4)+((j-1)*.087)) & tfile<(success_markers_times(i,4)+(((j-1)*.087)+.087)));
            success_timenorm_rates(loop,i,j+46)=length(x)*11.4943;
        end    
        ...
        for j=1:5
            ...
            success_timenorm_rates(loop,i,j+23)=length(x)*(1/trial_timebin);
        end
        % return
        trial_timebin=ww(i)/18;
        for j=1:18
            ...
            success_timenorm_rates(loop,i,j+28)=length(x)*(1/trial_timebin);
        end
    end
end
...
for loop=1:no_cells
    ...
    for i=1:length(failure_starts)
        ...
        for j=-23:-1
            ...
            failure_timenorm_rates(loop,i,j+24)=length(x)*11.4943;
        end
        % last 2 seconds
        for j=1:23
            x=find(tfile>(failure_markers_times(i,4)+((j-1)*.087)) & tfile<(failure_markers_times(i,4)+(((j-1)*.087)+.087)));
            failure_timenorm_rates(loop,i,j+46)=length(x)*11.4943;
        end    
        ...
        for j=1:5
            ...
            failure_timenorm_rates(loop,i,j+23)=length(x)*(1/trial_timebin);
        end
        % return
        trial_timebin=ww(i)/18;
        for j=1:18
            ...
            failure_timenorm_rates(loop,i,j+28)=length(x)*(1/trial_timebin);
        end
    end
end

success_timenorm_rates_smooth=zeros(no_cells,length(success_starts),69);
failure_timenorm_rates_smooth=zeros(no_cells,length(failure_starts),69);
...
success_timenorm_rates_smooth_means_stds_stes=zeros(no_cells,69,3);
failure_timenorm_rates_smooth_means_stds_stes=zeros(no_cells,69,3);
...
% diff_magdiffs_success_failure_mean_std_timenorm_rates=zeros(no_cells,69,4);
% 
% for loop=1:no_cells
...
% end    
% 
% absdiff_absmagdiffs_succ_fail_tnorm_crosscell_means_stds=zeros(69,4,2);
% 
% for i=1:69
%     for j=1:4
...
% success_timenorm_vels=zeros(length(success_starts),69);
% failure_timenorm_vels=zeros(length(failure_starts),69);
% 
% 
%     for i=1:length(success_starts)
%         for j=-10:-1
...
%     end
% 
% success_timenorm_vels_smooth=zeros(length(success_starts),69);
% failure_timenorm_vels_smooth=zeros(length(failure_starts),69);
% 
%     for i=1:no_successes
...
% success_timenorm_vels_smooth_means_stds_stes=zeros(69,3);
% failure_timenorm_vels_smooth_means_stds_stes=zeros(69,3);
% 
%     for i=1:69
...
% diff_magdiffs_success_failure_mean_std_timenorm_vels=zeros(69,4);
% 
