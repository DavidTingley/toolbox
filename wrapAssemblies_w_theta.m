% try
    ...
    if ~exist('assembliesCrossRegionData_w_ratemaps.mat') %&& ~exist('assemblies.mat')
    ...
    % generate continuous binned spike trains
        ...
        for c = 1:size(behavior.events.trialIntervals,1)
              ...
              r = makeLength(squeeze(firingMaps.rateMaps{cond}(cell,c,:)),length(train));
              
              ratemaps{behavior.events.trialConditions(c)} = ...
                  [ratemaps{behavior.events.trialConditions(c)},r];
        ...
        for c = 1:length(unique(behavior.events.trialConditions))
            extraPredictors = [phasetrains_sin{c};phasetrains_cos{c};phasetrains{c};coords{c};velocities{c};rateMaps{c}];
%             for p=1:length(pairs)
            [dev{c} devControl{c}] =cell_assembly_w_theta((spktrains{c}),0:200,extraPredictors,pairs);
            save('assembliesCrossRegionData_w_ratemaps.mat','spktrains','phasetrains','coords','velo*','pairs','dev*');
%             end
        end
%     elseif exist('assembliesCrossRegionData_w_ratemaps.mat') %&& ~exist('assemblies.mat')
%         disp('adding to pre-existing file')
%         pairs_all = pairs; clear pairs
%         load('assembliesCrossRegionData_w_ratemaps.mat','spktrains','pairs','dev*');
%         if exist('devControl')
...
%         end
    ...
    save('assembliesCrossRegionData_w_ratemaps.mat','dev*','pairs')
% catch
% end